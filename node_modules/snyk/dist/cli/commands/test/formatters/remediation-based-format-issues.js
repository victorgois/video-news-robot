...
function constructPatchesText(patches, basicVulnInfo) {
    ...
    for (const id of Object.keys(patches)) {
        ...
        const thisPatchFixes = formatIssue(id, basicVulnInfo[id].title, basicVulnInfo[id].severity, basicVulnInfo[id].isNew, `${basicVulnInfo[id].name}@${basicVulnInfo[id].version}`);
...
function constructUpgradesText(upgrades, basicVulnInfo) {
    ...
    for (const upgrade of Object.keys(upgrades)) {
        ...
        const thisUpgradeFixes = vulnIds
            .map((id) => formatIssue(id, basicVulnInfo[id].title, basicVulnInfo[id].severity, basicVulnInfo[id].isNew, `${basicVulnInfo[id].name}@${basicVulnInfo[id].version}`))
            .join('\n');
...
function formatIssue(id, title, severity, isNew, vulnerableModule) {
    ...
    return severitiesColourMapping[severity].colorFunc(`  ✗ ${chalk_1.default.bold(title)}${newBadge} [${titleCaseText(severity)} Severity]`) + `[${config.ROOT}/vuln/${id}]` + name;
}

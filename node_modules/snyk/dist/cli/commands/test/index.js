        ...
        for (const path of args) {
            ...
            for (let i = 0; i < resArray.length; i++) {
                ...
                if (!testOpts.subProjectNames) {
                    ...
                    resultOptions.push(_.assign(_.cloneDeep(testOpts), { subProjectName: testOpts.subProjectNames[i] }));
                }
            }
        }
...
function displayResult(res, options) {
    ...
    if (res.ok && res.vulnerabilities.length === 0) {
        ...
        // to consider
        const snykPackageTestTip = !(options.docker || localPackageTest || options.dev) ?
            '\n\nTip: Snyk only tests production dependencies by default. You can try re-running with the `--dev` flag.' : '';
        return (prefix + meta + summaryOKText + multiProjAdvice + (is_ci_1.isCI() ? '' :
            dockerAdvice +
                nextStepsText +
                snykPackageTestTip));
    }
    ...
    let summary = testedInfoText + ', ' + chalk_1.default.red.bold(vulnCountText);
    if (localPackageTest && package_managers_1.WIZARD_SUPPORTED_PACKAGE_MANAGERS.includes(packageManager)) {
...
function metaForDisplay(res, options) {
    ...
    if (options.subProjectName) {
        meta.push(chalk_1.default.bold(rightPadWithSpaces('Sub project: ', padToLength)) + options.subProjectName);
    }
    if (options.docker) {
...
function getSeverityValue(severity) {
    return common_1.SEVERITIES.find((severityObj) => severityObj.verboseName === severity).value;
}
